...
- **Bar and line charts.** A ` ```grafico ` (or ` ```chart `) block turns a small comma-, semicolon- or tab-separated table into a chart, drawn by the reader itself as SVG — no library, so it works offline too. It follows the theme, prints in light colors, shows every value on hover, and has a **Ver datos** table view.
