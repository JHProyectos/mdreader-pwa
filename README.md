...
- **Syntax highlighting.** Code blocks tagged with JavaScript, TypeScript, Python, HTML, CSS, Java, C or C# are highlighted with [Prism](https://prismjs.com/), loaded on demand. Colors come from the reader's own theme. Without a connection, or for any other language, the block is shown in a single color. Every block has a button at its top right corner that copies the whole snippet to the clipboard.
...
- **Resaltado de código.** Los bloques con lenguaje JavaScript, TypeScript, Python, HTML, CSS, Java, C o C# se resaltan con [Prism](https://prismjs.com/), que se carga a demanda. Los colores son los del tema del lector. Sin conexión, o con otro lenguaje, el bloque se ve en un solo color. Cada bloque tiene arriba a la derecha un botón que lo copia entero al portapapeles.
